...
function settings(baseUrl: string, over: Partial<{ requestTimeoutMs: number; maxRetries: number; retryBackoffMs: number; thinkEffort?: "low" | "medium" | "high" | boolean }> = {}) {
  return SettingsSchema.parse({ providers: [{ providerName: "lm-studio", baseUrl, ...over }] }).providers.find(p => p.providerName === "lm-studio");
}
...
test("chat accumulates streamed content and tool_calls", async () => {
  ...
  try {
    const client = new LMStudioClient(settings(`http://127.0.0.1:${port}`) as Provider);
    const seen: number[] = [];
...
test("chat reports a dropped connection instead of a bare 'fetch failed'", async () => {
  const server = createServer((_req, res) => {
    res.writeHead(200, { "Content-Type": "application/x-ndjson" });
    res.write(JSON.stringify({ choices: [{ delta: { content: "partial" }, finish_reason: null }] }) + "\n");
...
test("chat respects requestTimeoutMs", async () => {
  ...
  const port = (server.address() as AddressInfo).port;
  try {
    const client = new LMStudioClient(settings(`http://127.0.0.1:${port}`, { requestTimeoutMs: 200 }) as Provider);
