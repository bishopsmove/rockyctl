...
import { sleep, ndjsonLines, describeError, isRetryableError } from "./utils.js";

export class UnslothError extends Error {
  constructor(message: string, public status?: number) {
    super(message);
    this.name = "UnslothError";
  }
}
 ...
 */
export class UnslothClient implements HostProvider {
  ...
  private async chatOnce(
    ...
    try {
      ...
      if (!res.body) throw new UnslothError("POST /v1/chat/completions returned no body");
      ...
      for await (const line of ndjsonLines(res.body as unknown as ReadableStream<Uint8Array>, controller.signal)) {
        ...
        if (chunk.error) throw new UnslothError(`Unsloth error: ${chunk.error}`);
      ...
      if (!final) {
        throw new UnslothError(`Stream from ${model} ended without a final chunk (connection dropped?)`);
      }
      ...
      if (controller.signal.aborted) {
        throw new UnslothError(`Generation with ${model} exceeded requestTimeoutMs (${this.provider.requestTimeoutMs}ms)`);
      }
      if (err instanceof UnslothError) throw err;
      throw new UnslothError(`Streaming from ${model} failed: ${describeError(err)}`);
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async generate(
    ...
    if (opts.system) body.prompt = `${opts.system}\n\n${prompt}`;
    ...
    try {
      ...
      if (result.error) throw new UnslothError(`Unsloth error during generation: ${result.error}`);
      ...
      if (err instanceof UnslothError) throw err;
      throw new UnslothError(`Generation with ${model} failed: ${describeError(err)}`);
    }
  }
  ...
  async waitUntilReady(models: string[], progress: ProgressFn = () => {}): Promise<void> {
    ...
    progress(`Contacting Unsloth Desktop at ${this.provider.baseUrl} ...`);
    let connected = false;
    let lastErr: unknown;
    while (remaining() > 0) {
      try {
        // Unsloth Desktop (OpenAI compatible) might not have /health
        // but /v1/models is a good probe.
        await this.fetch("/v1/models", { method: "GET" }, Math.min(5_000, remaining()));
        connected = true;
        break;
      } catch (err) {
        lastErr = err;
    ...
    if (!connected) {
      throw new UnslothError(
        `Unsloth Desktop at ${this.provider.baseUrl} did not respond within ${this.provider.readyTimeoutMs}ms` +
  ...
  private async fetch(
    ...
    try {
      ...
      if (!res.ok) {
        throw new UnslothError(`${init.method} ${path} -> HTTP ${res.status}`, res.status);
      }
      return res;
    } catch (err) {
      if (controller.signal.aborted) {
        throw new UnslothError(`${init.method} ${path} timed out after ${timeoutMs}ms`);
      }
      throw err;
    } finally {
