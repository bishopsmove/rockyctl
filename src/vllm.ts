 ...
 */
export class VLLMClient implements HostProvider {
  ...
  private async chatOnce(
    ...
    try {
      ...
      if (!res.body) throw new VLLMError("POST /v1/chat/completions returned no body");
      ...
      for await (const line of ndjsonLines(res.body as unknown as ReadableStream<Uint8Array>, controller.signal)) {
        ...
        if (chunk.error) throw new VLLMError(`vLLM error: ${chunk.error}`);
      ...
      if (controller.signal.aborted) {
        throw new VLLMError(`Generation with ${model} exceeded requestTimeoutMs (${this.provider.requestTimeoutMs}ms)`);
      }
      throw err;
    } finally {
      clearTimeout(timer);
  ...
  async waitUntilReady(models: string[], progress: ProgressFn = () => {}): Promise<void> {
    ...
    progress(`Contacting vLLM at ${this.provider.baseUrl} ...`);
    let connected = false;
    let lastErr: unknown;
    while (remaining() > 0) {
      try {
    ...
    if (!connected) {
      throw new VLLMError(
        `vLLM at ${this.provider.baseUrl} did not respond within ${this.provider.readyTimeoutMs}ms` +
  ...
  private async fetch(
    ...
    try {
      ...
      if (!res.ok) {
        throw new VLLMError(`${init.method} ${path} -> HTTP ${res.status}`, res.status);
      }
      return res;
    } catch (err) {
      if (controller.signal.aborted) {
        throw new VLLMError(`${init.method} ${path} timed out after ${timeoutMs}ms`);
      }
      throw err;
    } finally {
